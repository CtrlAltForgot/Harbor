...
describe("safe organizer",()=>{
  ...
  it("renames the primary video and matching subtitle inside a movie directory",async()=>{const root=mkdtempSync(path.join(tmpdir(),"harbor-org-"));roots.push(root);const source=path.join(root,"release"),destination=path.join(root,"movies");await mkdir(source);writeFileSync(path.join(source,"release.1080p.mkv"),"video");writeFileSync(path.join(source,"release.1080p.en.srt"),"subs");const result=await organize(source,destination,{category:"movie",confidence:1,reasons:[],title:"Example (2025)"});expect(readFileSync(path.join(result.destination,"Example (2025).mkv"),"utf8")).toBe("video");expect(readFileSync(path.join(result.destination,"Example (2025).en.srt"),"utf8")).toBe("subs");});
});
