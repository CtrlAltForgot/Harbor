...
export class QbitClient {
  ...
  async addMagnet(magnet:string, _category:string){const form=new FormData();form.set("urls",magnet);form.set("savepath",this.config.incompleteDir);await this.expectAccepted("/api/v2/torrents/add",form);}
  async addFile(data:Buffer,fileName:string,_category:string){const form=new FormData();form.set("torrents",new Blob([new Uint8Array(data)]),fileName);form.set("savepath",this.config.incompleteDir);await this.expectAccepted("/api/v2/torrents/add",form);}
  ...
  async action(hash:string,action:"pause"|"resume"|"recheck"){const modern=action==="pause"?"stop":action==="resume"?"start":"recheck";try{await this.post(`/api/v2/torrents/${modern}`,{hashes:hash});}catch(error){if(action==="recheck")throw error;await this.post(`/api/v2/torrents/${action}`,{hashes:hash});}}
  async remove(hash:string,deleteFiles=false){await this.post("/api/v2/torrents/delete",{hashes:hash,deleteFiles:String(deleteFiles)});}
