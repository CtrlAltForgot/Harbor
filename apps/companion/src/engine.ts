...
export function parseMagnet(value: string) {
  ...
  try {
    ...
    throw new Error("The magnet link is malformed");
  }
  if (url.protocol !== "magnet:")
    throw new Error("Only magnet links are accepted");
  const xt = url.searchParams.get("xt") ?? "";
  const match = xt.match(/^urn:btih:([a-z0-9]+)$/i);
  if (!match)
    throw new Error("The magnet link has no valid BitTorrent info hash");
  const raw = match[1]!;
  if (!/^[a-f0-9]{40}$/i.test(raw) && !/^[a-z2-7]{32}$/i.test(raw))
    throw new Error("The magnet link has an invalid BitTorrent v1 info hash");
  const hash = raw.length === 32 ? decodeBase32(raw) : raw.toLowerCase();
  return {
    hash,
    name: url.searchParams.get("dn") || `Torrent ${hash.slice(0, 8)}`,
  };
}
export function createTorrent(
  input: AddTorrentRequest,
  destinations: Record<string, string>,
): Torrent {
  let infoHash: string, name: string, source: Torrent["source"];
  if (input.magnet) {
    ({ hash: infoHash, name } = parseMagnet(input.magnet));
    source = "magnet";
  } else if (input.torrentBase64) {
    const metadata = parseTorrentFile(
  ...
  return {
    ...
    destination: destinations[category] ?? destinations.review!,
    retention: input.retention ?? "remove",
    createdAt: new Date().toISOString(),
    files: [{ id: 0, name, size: 2_400_000_000, progress: 0, priority: 1 }],
  };
}
...
export function tick(torrent: Torrent): Torrent {
  ...
  return {
    ...
    downloadSpeed: completed ? 0 : 8_400_000,
    uploadSpeed: completed ? 460_000 : 90_000,
    etaSeconds: completed
      ? 0
      : Math.ceil((torrent.size * (1 - progress)) / 8_400_000),
    seeds: 24,
    peers: 8,
...
function decodeBase32(value: string) {
  ...
  for (const character of value.toUpperCase()) {
    ...
    if (index < 0) throw new Error("Invalid base32 info hash");
