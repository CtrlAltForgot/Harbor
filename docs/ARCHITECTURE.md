...
qBittorrent is the intended production torrent engine. Harbor controls its Web API and does not implement BitTorrent. The deterministic mock engine is the current runnable adapter and enables UI/testing without network traffic. Shared TypeScript contracts keep the UI and server aligned.
...
The organizer will journal a plan before touching files, verify the target, then apply retention. Cross-filesystem operations copy to a temporary target, fsync, verify size/hash policy, and atomically rename. Hardlinks are opt-in and capability-checked. Low-confidence classification routes to review.
